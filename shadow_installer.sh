...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='shadow.conf'
CONFIGFOLDER='/root/.shadow'
COIN_DAEMON='shadowd'
COIN_CLI='shadow-cli'
COIN_PATH='/usr/local/bin/'
COIN_REPO='https://github.com/shadowcoin/shadow.git'
COIN_TGZ='https://github.com/shadowcoin/shadow/releases/download/v3.0.0.0/mirq-3.0.0-x86_64-linux-signed.tar.gz'
...
function get_ip() {
  declare -a NODE_shadow
  for shadow in $(netstat -i | awk '!/Kernel|Iface|lo/ {print $1," "}')
  do
    NODE_shadow+=($(curl --interface $shadow --connect-timeout 2 -s4 icanhazip.com))
  done
  ...
  if [ ${#NODE_shadow[@]} -gt 1 ]
    then
      ...
      INDEX=0
      for ip in "${NODE_shadow[@]}"
      do
        echo ${INDEX} $ip
        let INDEX=${INDEX}+1
      done
      read -e choose_ip
      NODEIP=${NODE_shadow[$choose_ip]}
  else
    NODEIP=${NODE_shadow[0]}
  fi
}
